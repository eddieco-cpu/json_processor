...
//
async function dealWithJsonUrl(
	url: string
): Promise<[JSONValue | null, string | null]> {
	if (url.length > 120000) {
		return [
			null,
			`Search params' length is too large, must be less than 120000. (${url.length})`,
		];
	}
	try {
...
function dealWithJsonData(str: string): [JSONValue | null, string | null] {
	if (str.length > 120000) {
		return [
			null,
			`Search params' length is too large, must be less than 120000. (${str.length})`,
		];
	}
	try {
...
export default async function page({
	 ...
	 */
	let [receivedJson, receivedJsonError]: [JSONValue | null, string | null] = [
		null,
		null,
	];
	...
	return (
		<section>
			<Suspense fallback={<div>Loading...</div>}>
				<Panel
					insertedJson={receivedJson}
					insertedJsonError={receivedJsonError}
				/>
			</Suspense>
		</section>
	);
