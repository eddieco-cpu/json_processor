...
export default async function page({
	params: { jsonStorageID },
}: {
	params: { jsonStorageID: string };
}) {
	//
	...
	//
	const insertedJson = receivedJson
		? JSON.parse(JSON.stringify(receivedJson))
		: null;
	const insertedJsonError = insertedJson
		? null
	...
	return (
		<section>
			<Suspense fallback={<div>Loading...</div>}>
				<Panel
					insertedJson={insertedJson}
					insertedJsonError={insertedJsonError}
				/>
			</Suspense>
		</section>
	);
